...
def get_cifar10_cnn(inputShape):
    ...
    num_classes = 10
    model = Sequential()
    # In the first CONV layer, the filter size needs to be 5*5 and the stride should be 1. 
    # The total number of filters in the first Convolutional layer should be 32

    # Layer #1: Convolution layer 1, Size (32x32x3) -> (28x28x32)
    ...
    # Layer #2: Convolution layer 2, Size (28x28x32) -> (28x28x64)
    model.add(Conv2D(64, (3, 3), padding='same',kernel_regularizer=regularizers.l2(weight_decay)))
    model.add(Activation('relu'))
    model.add(BatchNormalization())

    # Layer #3: Max Pooling layer 1 (28x28x64) -> (14x14x64)
    model.add(MaxPooling2D(pool_size=(2, 2),strides=2))

    # Layer #4: Convolution layer 3, Size (14x14x64) -> (10x10x128)
    model.add(Conv2D(128, (5, 5), padding='valid',kernel_regularizer=regularizers.l2(weight_decay)))
    model.add(Activation('relu'))
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    # Layer #5: Convolution layer 4, Size (10x10x64) -> (10x10x256)
    model.add(Conv2D(256, (3, 3), padding='same',kernel_regularizer=regularizers.l2(weight_decay)))
    model.add(Activation('relu'))
    model.add(BatchNormalization())

    # Layer #6: Max Pooling layer 2, Size (10x10x256) -> (5x5x256)
    model.add(MaxPooling2D(pool_size=(2, 2),strides=2))

    # Layer #7: Convolution layer 5, Size (5x5x256) -> (3x3x512)
    model.add(Conv2D(512, (3, 3), padding='valid',kernel_regularizer=regularizers.l2(weight_decay)))
    model.add(Activation('relu'))
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    # Layer #8: Convolution layer 6, Size (3x3x512) -> (1x1x1024)
    model.add(Conv2D(1024, (3, 3), padding='valid',kernel_regularizer=regularizers.l2(weight_decay)))
    model.add(Activation('relu'))
    model.add(BatchNormalization())
    model.add(Dropout(0.4))
